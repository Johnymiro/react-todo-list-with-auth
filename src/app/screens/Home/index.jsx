...
function HomePage(props) {
  ...
  // eslint-disable-next-line react-hooks/rules-of-hooks
  const [selectedListEditItemIndex, setSelectedListEditItemIndex] = useState(
    3.14
  );
  // eslint-disable-next-line react-hooks/rules-of-hooks
  ...
  const saveEditTodo = (title, detail, date) => {
    ...
    if (title !== "" && detail !== "") {
      ...
      selectedList.todoList[selectedListEditItemIndex].date = new Date(
        date
      ).getTime();
      ...
      return (
        <Dialog open={showEditTodoDialog} aria-labelledby="form-dialog-title">
          ...
          <DialogActions>
            ...
            <Button
              onClick={() => saveEditTodo(title, detail, date)}
              color="primary"
            >
              Save
            </Button>
      ...
      return todoLists.map((list, index) => {
        return (
          ...
          >
            ...
            <IconButton
              aria-label="delete"
              className={classes.listRemoveButton}
              onClick={(event) => {
